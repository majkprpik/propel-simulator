...
crudRoutes.post('/api/accounts', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    account_id: parsed.data.account_id || `${Math.floor(Math.random() * 9000000000) + 1000000000}`,
...
crudRoutes.post('/api/campaigns', async (c) => {
  ...
  const record = {
    ...
    campaign_id: parsed.data.campaign_id || `${Math.floor(Math.random() * 90000000000) + 10000000000}`,
    name: parsed.data.name,
    objective: parsed.data.objective || 'SEARCH',
...
crudRoutes.post('/api/ad-groups', async (c) => {
  ...
  const record = {
    ...
    ad_group_id: parsed.data.ad_group_id || `${Math.floor(Math.random() * 90000000000) + 10000000000}`,
    name: parsed.data.name,
    status: parsed.data.status || 'ENABLED',
...
crudRoutes.post('/api/ads', async (c) => {
  ...
  const record = {
    ...
    ad_id: `${Math.floor(Math.random() * 90000000000) + 10000000000}`,
    name: parsed.data.name,
...
// Pixels (Conversion Actions in Google terminology)
...
crudRoutes.post('/api/pixels', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    ad_account_id: parsed.data.ad_account_id,
    pixel_id: parsed.data.pixel_id || `customers/${parsed.data.ad_account_id}/conversionActions/${Math.floor(Math.random() * 900000000) + 100000000}`,
    name: parsed.data.name,
