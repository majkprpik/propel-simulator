...
crudRoutes.post('/api/accounts', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    account_id: parsed.data.account_id || crypto.randomUUID(),
...
crudRoutes.post('/api/campaigns', async (c) => {
  ...
  const record = {
    ...
    campaign_id: parsed.data.campaign_id || crypto.randomUUID(),
    name: parsed.data.name,
    objective: parsed.data.objective || 'WEB_CONVERSION',
    status: parsed.data.status || 'ACTIVE',
...
// Ad Groups (Ad Squads in Snapchat terminology)
...
crudRoutes.post('/api/ad-groups', async (c) => {
  ...
  const record = {
    ...
    ad_group_id: parsed.data.ad_group_id || crypto.randomUUID(),
    name: parsed.data.name,
    status: parsed.data.status || 'ACTIVE',
...
crudRoutes.post('/api/ads', async (c) => {
  ...
  const record = {
    ...
    ad_id: `snap_ad_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
...
crudRoutes.post('/api/pixels', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    ad_account_id: parsed.data.ad_account_id,
    pixel_id: parsed.data.pixel_id || crypto.randomUUID(),
    name: parsed.data.name,
    access_token: parsed.data.access_token || `snap_mock_${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`,
    status: parsed.data.status || 'ACTIVE',
  };
