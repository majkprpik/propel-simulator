...
crudRoutes.post('/api/accounts', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    account_id: parsed.data.account_id || `nb_${crypto.randomUUID().slice(0, 12)}`,
...
crudRoutes.post('/api/campaigns', async (c) => {
  ...
  const record = {
    ...
    campaign_id: parsed.data.campaign_id || `nb_camp_${crypto.randomUUID().slice(0, 10)}`,
    name: parsed.data.name,
    objective: parsed.data.objective || 'TRAFFIC',
    status: parsed.data.status || 'active',
...
crudRoutes.post('/api/ad-groups', async (c) => {
  ...
  const record = {
    ...
    ad_group_id: parsed.data.ad_group_id || `nb_ad_${crypto.randomUUID().slice(0, 10)}`,
    name: parsed.data.name,
    status: parsed.data.status || 'active',
...
crudRoutes.post('/api/ads', async (c) => {
  ...
  const record = {
    ...
    ad_id: `nb_ad_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
...
crudRoutes.post('/api/pixels', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    ad_account_id: parsed.data.ad_account_id,
    pixel_id: parsed.data.pixel_id || `nb_px_${crypto.randomUUID().slice(0, 10)}`,
    name: parsed.data.name,
    access_token: parsed.data.access_token || `nb_token_${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`,
    status: parsed.data.status || 'active',
  };
