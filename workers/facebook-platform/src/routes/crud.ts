...
crudRoutes.post('/api/accounts', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    account_id: parsed.data.account_id || `act_${crypto.randomUUID().slice(0, 12)}`,
...
crudRoutes.post('/api/campaigns', async (c) => {
  ...
  const record = {
    ...
    campaign_id: parsed.data.campaign_id || `camp_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
    objective: parsed.data.objective || 'CONVERSIONS',
    status: parsed.data.status || 'ACTIVE',
...
// Ad Groups (Ad Sets in Facebook terminology)
...
crudRoutes.post('/api/ad-groups', async (c) => {
  ...
  const record = {
    ...
    ad_group_id: parsed.data.ad_group_id || `adset_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
    status: parsed.data.status || 'ACTIVE',
...
crudRoutes.post('/api/ads', async (c) => {
  ...
  const record = {
    ...
    ad_id: `fb_ad_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
...
crudRoutes.post('/api/pixels', async (c) => {
  ...
  const record = {
    platform: PLATFORM,
    ad_account_id: parsed.data.ad_account_id,
    pixel_id: parsed.data.pixel_id || `px_${crypto.randomUUID().slice(0, 12)}`,
    name: parsed.data.name,
    access_token: parsed.data.access_token || `EAAMock${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`,
    status: parsed.data.status || 'active',
  };
