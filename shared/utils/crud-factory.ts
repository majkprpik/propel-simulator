...
import { createCrudRateLimiter } from './rate-limiter';

export const SUPABASE_NOT_FOUND = 'PGRST116';

/** Supabase upsert conflict target for deduplicating events by platform + event_id */
export const EVENT_UPSERT_CONFLICT = 'platform,event_id' as const;

import type { WorkerBindings, WorkerAppType } from '../types/worker';
...
export function createCrudRoutes(options: CrudFactoryOptions): Hono<WorkerAppType> {
  const { platform, generateClickId, getDb, isRateLimitEnabled, defaults } = options;
  const accountStatus = defaults.accountStatus ?? 'active';
  const pixelStatus = defaults.pixelStatus ?? 'active';
  const crudRoutes = new Hono<WorkerAppType>();

  // Rate-limit write operations (POST/PUT/DELETE) on CRUD endpoints
  crudRoutes.use('*', createCrudRateLimiter({ isForceEnabled: isRateLimitEnabled }));
  ...
  crudRoutes.post('/api/accounts', async (c) => {
    ...
    const record = {
      platform,
      account_id: parsed.data.account_id || defaults.generateAccountId(),
      name: parsed.data.name,
      currency: parsed.data.currency || 'USD',
      status: parsed.data.status || accountStatus,
    };
  ...
  crudRoutes.post('/api/campaigns', async (c) => {
    ...
    const record = {
      ...
      status: parsed.data.status || defaults.campaignStatus,
  ...
  crudRoutes.post('/api/ad-groups', async (c) => {
    ...
    const record = {
      ...
      ad_group_id: parsed.data.ad_group_id || defaults.generateAdGroupId(),
      name: parsed.data.name,
      status: parsed.data.status || defaults.adGroupStatus,
  ...
  crudRoutes.post('/api/ads', async (c) => {
    ...
    const record = {
      ...
      ad_id: defaults.generateAdId(),
      name: parsed.data.name,
      destination_url: parsed.data.destination_url || null,
      status: parsed.data.status || defaults.adStatus,
    };
  ...
  crudRoutes.post('/api/pixels', async (c) => {
    ...
    const record = {
      ...
      pixel_id: parsed.data.pixel_id || defaults.generatePixelId(),
      name: parsed.data.name,
      access_token: parsed.data.access_token || defaults.generateAccessToken(),
      status: parsed.data.status || pixelStatus,
    };
