...
export function SnapchatSettings() {
  const [postbackUrl, setPostbackUrl] = useState('');
  const [eventName, setEventName] = useState('PURCHASE');
  const [accountId, setAccountId] = useState('');

  const queryClient = useQueryClient();
  const configs = useQuery({ queryKey: ['snapchat', 'postback-configs'], queryFn: () => listResource<MockPostbackConfig>('snapchat', 'postback-configs') });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('snapchat', 'postback-configs', body),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['snapchat', 'postback-configs'] }); setPostbackUrl(''); setEventName('PURCHASE'); setAccountId(''); },
  });
    ...
    <div>
      ...
      <div className="mb-8 rounded-lg border border bg-card p-6">
        ...
        <div className="space-y-2 text-sm">
          <p><span className="font-medium text-muted-foreground">CAPI Endpoint:</span> <code className="rounded bg-accent px-2 py-0.5">POST http://localhost:8805/v2/conversion</code></p>
          <p><span className="font-medium text-muted-foreground">Ads API:</span> <code className="rounded bg-accent px-2 py-0.5">GET http://localhost:8805/v1/...</code></p>
          <p><span className="font-medium text-muted-foreground">Port:</span> 8805</p>
          <p><span className="font-medium text-muted-foreground">Platform:</span> snapchat</p>
        </div>
      </div>
      <div className="rounded-lg border border bg-card p-6">
        ...
        <div className="mb-6 space-y-3">
          <label className="block"><span className="text-sm font-medium text-card-foreground">Ad Account ID</span><input value={accountId} onChange={(e) => setAccountId(e.target.value)} className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none" /></label>
          <label className="block"><span className="text-sm font-medium text-card-foreground">Event Type</span><select value={eventName} onChange={(e) => setEventName(e.target.value)} className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none"><option>PURCHASE</option><option>SIGN_UP</option><option>ADD_CART</option></select></label>
