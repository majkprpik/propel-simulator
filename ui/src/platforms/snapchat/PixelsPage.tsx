...
export function SnapchatPixels() {
  const [dialogOpen, setDialogOpen] = useState(false);
  const [name, setName] = useState('');
  const [accountId, setAccountId] = useState('');

  const queryClient = useQueryClient();
  const { data, isLoading, isError, error } = useQuery({ queryKey: ['snapchat', 'pixels'], queryFn: () => listResource<MockPixel>('snapchat', 'pixels') });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('snapchat', 'pixels', body),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['snapchat', 'pixels'] }); setDialogOpen(false); setName(''); setAccountId(''); },
  });
  ...
  return (
    <div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Create Pixel" onSubmit={() => create.mutate({ name, ad_account_id: accountId })}>
        ...
        <label className="block"><span className="text-sm font-medium text-card-foreground">Ad Account ID</span><input value={accountId} onChange={(e) => setAccountId(e.target.value)} required className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none" /></label>
      </FormDialog>
    </div>
  );
}
