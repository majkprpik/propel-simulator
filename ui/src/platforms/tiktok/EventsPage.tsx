...
const columns: Column<MockEvent>[] = [
  ...
  { key: 'click_id', label: 'TTCLID', render: (v) => v ? String(v).slice(0, 12) + '...' : '-' },
...
export function TikTokEvents() {
  ...
  const events = useQuery({ queryKey: ['tiktok', 'events', pixelFilter], queryFn: () => getEvents('tiktok', pixelFilter || undefined) });
