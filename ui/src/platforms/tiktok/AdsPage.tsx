...
export function TikTokAds() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['tiktok', 'ads'],
    queryFn: () => listResource<MockAd>('tiktok', 'ads'),
  });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('tiktok', 'ads', body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tiktok', 'ads'] });
      queryClient.invalidateQueries({ queryKey: ['tiktok', 'active-ads'] });
      setDialogOpen(false);
      setName('');
      setUrl('');
      setAdGroupId('');
