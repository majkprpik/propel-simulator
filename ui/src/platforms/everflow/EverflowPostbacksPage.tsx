...
import type { EfPostback } from '@shared/types/database';

export function EverflowPostbacksPage() {
  ...
  const clearAll = useMutation({
    mutationFn: () => everflowFetch('/api/postbacks', { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['everflow', 'postbacks'] }),
  });
    ...
    <div>
      ...
      {postbacks.isLoading ? (
        ...
        <div className="rounded-lg border border-dashed p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Propel will send <code className="rounded bg-muted px-1">GET /conversions?transaction_id=...</code> here after a conversion.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Propel Click ID (adv1)</th>
            ...
            <tbody className="divide-y">
              {(postbacks.data?.data ?? []).map((pb) => (
                <tr key={pb.id} className={`hover:bg-muted/30 ${pb.click_matched ? 'bg-green-50/30' : 'bg-yellow-50/30'}`}>
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{pb.adv1 ?? '—'}</td>
