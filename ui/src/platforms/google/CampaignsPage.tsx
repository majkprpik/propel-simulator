...
export function GoogleCampaigns() {
  ...
  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('google', 'campaigns', body),
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      >
        ...
        <label className="block">
          <span className="text-sm font-medium text-card-foreground">Campaign Type</span>
          <select value={objective} onChange={(e) => setObjective(e.target.value)} className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none">
            <option>SEARCH</option>
            <option>DISPLAY</option>
            <option>PERFORMANCE_MAX</option>
          </select>
        </label>
        <label className="block">
          <span className="text-sm font-medium text-card-foreground">Daily Budget ($)</span>
          <input type="number" value={budget} onChange={(e) => setBudget(e.target.value)} className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
      </FormDialog>
    </div>
  );
