...
const columns: Column<MockAdGroup>[] = [
  ...
  { key: 'bid_amount', label: 'CPC Bid', render: (v) => (v != null ? `$${v}` : '-') },
  { key: 'created_at', label: 'Created', render: (v) => formatDate(v as string) },
];

export function GoogleAdGroups() {
  ...
  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('google', 'ad-groups', body),
    onSuccess: () => {
    ...
    <div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Create Ad Group" onSubmit={() => create.mutate({ name, campaign_id: campaignId, bid_amount: bid ? Number(bid) : null })}>
        ...
        <label className="block">
          ...
          <input value={campaignId} onChange={(e) => setCampaignId(e.target.value)} required className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
        <label className="block">
          <span className="text-sm font-medium text-card-foreground">CPC Bid ($)</span>
          <input type="number" value={bid} onChange={(e) => setBid(e.target.value)} className="mt-1 block w-full rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
      </FormDialog>
    </div>
  );
