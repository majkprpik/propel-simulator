...
const columns: Column<MockEvent>[] = [
  ...
  { key: 'pixel_id', label: 'Conversion Action' },
  { key: 'click_id', label: 'GCLID', render: (v) => v ? String(v).slice(0, 12) + '...' : '-' },
...
export function GoogleEvents() {
  ...
  const events = useQuery({
    queryKey: ['google', 'events', pixelFilter],
    queryFn: () => getEvents('google', pixelFilter || undefined),
  });
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <select value={pixelFilter} onChange={(e) => setPixelFilter(e.target.value)} className="rounded-md border border px-3 py-2 text-sm focus:border-primary focus:outline-none">
          <option value="">All Conversion Actions</option>
