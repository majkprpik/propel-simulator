...
export function FacebookAds() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => listResource<MockAd>('facebook', 'ads'),
  });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('facebook', 'ads', body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['facebook', 'ads'] });
      queryClient.invalidateQueries({ queryKey: ['facebook', 'active-ads'] });
      setDialogOpen(false);
      setName('');
      setUrl('');
      setAdGroupId('');
  ...
  return (
    <div>
      ...
      >
        ...
        <label className="block">
          <span className="text-sm font-medium text-card-foreground">Ad Set ID (optional)</span>
          <input
            value={adGroupId}
