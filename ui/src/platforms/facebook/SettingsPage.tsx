...
export function FacebookSettings() {
  ...
  const configs = useQuery({
    queryKey: ['facebook', 'postback-configs'],
    queryFn: () => listResource<MockPostbackConfig>('facebook', 'postback-configs'),
  });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      createResource('facebook', 'postback-configs', body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['facebook', 'postback-configs'] });
      setPostbackUrl('');
    ...
    <div>
      ...
      <div className="mb-8 rounded-lg border border bg-card p-6">
        ...
        <div className="space-y-2 text-sm">
          <p>
            <span className="font-medium text-muted-foreground">CAPI Endpoint:</span>{' '}
            <code className="rounded bg-accent px-2 py-0.5">
              POST http://localhost:8801/:version/:pixel_id/events
            </code>
          </p>
          <p>
            <span className="font-medium text-muted-foreground">Port:</span> 8801
          </p>
          <p>
            <span className="font-medium text-muted-foreground">Platform:</span> facebook
          </p>
        </div>
      </div>
          ...
          <button
            onClick={() =>
              create.mutate({
                ...
                name: `${eventName} Postback`,
                postback_url: postbackUrl,
                event_name: eventName,
              })
            }
            disabled={!postbackUrl || !accountId}
          ...
          <div>
            ...
            <div className="space-y-2">
              {configs.data.data.map((cfg) => (
                <div
                  key={cfg.id}
                  className="flex items-center justify-between rounded border border-gray-100 bg-muted px-4 py-2 text-sm"
                >
                  <span>
                    {cfg.event_name} → <code className="text-xs">{cfg.postback_url}</code>
                  </span>
                  <span
                    className={`text-xs ${cfg.is_active ? 'text-green-600' : 'text-muted-foreground'}`}
                  >
