...
export function FacebookPixels() {
  ...
  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('facebook', 'pixels', body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['facebook', 'pixels'] });
      setDialogOpen(false);
      setName('');
      setAccountId('');
    },
  ...
  return (
    <div>
      ...
      <FormDialog
        ...
        title="Create Pixel"
        onSubmit={() => create.mutate({ name, ad_account_id: accountId })}
      >
        <label className="block">
          <span className="text-sm font-medium text-card-foreground">Pixel Name</span>
          <input
            value={name}
