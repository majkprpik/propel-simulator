...
export function ClickBankPostbacksPage() {
  ...
  const clearAll = useMutation({
    mutationFn: () => clickbankFetch('/api/postbacks', { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['clickbank', 'postbacks'] }),
  });
    ...
    <div>
      ...
      {postbacks.isLoading ? (
        ...
        <div className="rounded-lg border border-dashed p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Propel will send <code className="rounded bg-muted px-1">GET /conversions?receipt=...&cbpop=...</code> here after a conversion.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {(postbacks.data?.data ?? []).map((pb) => (
                <tr key={pb.id} className={`hover:bg-muted/30 ${pb.order_matched ? 'bg-green-50/30' : 'bg-yellow-50/30'}`}>
      ...
      {(postbacks.data?.total ?? 0) > 0 && (
        <p className="mt-2 text-xs text-muted-foreground">
          ...
          Green = receipt matched a known order. Yellow = unknown receipt.
        </p>
      )}
    </div>
  );
