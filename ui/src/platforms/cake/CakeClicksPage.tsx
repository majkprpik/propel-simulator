...
import type { CakeClick, CakeOffer } from '@shared/types/database';

export function CakeClicksPage() {
  ...
  const offers = useQuery({
    ...
    queryFn: () => cakeFetch<{ data: CakeOffer[] }>('/api/offers'),
  });

  const clicks = useQuery({
    queryKey: ['cake', 'clicks'],
    queryFn: () => cakeFetch<{ data: CakeClick[]; total: number }>('/api/clicks'),
    refetchInterval: 5000,
  });

  const generate = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      cakeFetch('/api/clicks/generate', { method: 'POST', body: JSON.stringify(body) }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['cake', 'clicks'] });
      setShowModal(false);
  ...
  const deleteClick = useMutation({
    mutationFn: (id: string) => cakeFetch(`/api/clicks/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['cake', 'clicks'] }),
  });
            ...
            <label className="mb-3 block">
              ...
              <select value={genForm.offer_id} onChange={(e) => setGenForm({ ...genForm, offer_id: e.target.value })} className="mt-1 block w-full rounded-md border px-3 py-2 text-sm focus:border-primary focus:outline-none">
                ...
                {(offers.data?.data ?? []).map((o) => (
                  <option key={o.offer_id} value={String(o.offer_id)}>{o.offer_name}</option>
                ))}
              </select>
            </label>
              ...
              {(clicks.data?.data ?? []).map((click) => (
                <tr key={click.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    {(click as CakeClick & { mock_cake_offers?: { offer_name: string } }).mock_cake_offers?.offer_name ?? click.offer_id ?? '—'}
                  </td>
