...
import type { CakePostback } from '@shared/types/database';

export function CakePostbacksPage() {
  ...
  const clearAll = useMutation({
    mutationFn: () => cakeFetch('/api/postbacks', { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['cake', 'postbacks'] }),
  });
    ...
    <div>
      ...
      {postbacks.isLoading ? (
        ...
        <div className="rounded-lg border border-dashed p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Propel will send <code className="rounded bg-muted px-1">GET /pixel.track?s1=...</code> here after a conversion.</p>
        </div>
      ) : (
      ...
      {(postbacks.data?.total ?? 0) > 0 && (
        <p className="mt-2 text-xs text-muted-foreground">
          ...
          Green = s1 matched a known click. Yellow = unknown s1.
        </p>
      )}
    </div>
  );
