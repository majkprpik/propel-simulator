...
export function NewsBreakAds() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['newsbreak', 'ads'],
    queryFn: () => listResource<MockAd>('newsbreak', 'ads'),
  });

  const create = useMutation({
    mutationFn: (body: Record<string, unknown>) => createResource('newsbreak', 'ads', body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['newsbreak', 'ads'] });
      queryClient.invalidateQueries({ queryKey: ['newsbreak', 'active-ads'] });
      setDialogOpen(false);
      setName('');
      setUrl('');
      setAdGroupId('');
